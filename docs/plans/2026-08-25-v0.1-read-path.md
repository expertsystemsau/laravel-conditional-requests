...
This is expected to pass without production changes. Laravel's `Router::runRoute()` calls `prepareResponse()` *after* the route middleware stack unwinds, and that is where Symfony's `Response::prepare()` nulls the body for `HEAD` — so route middleware still sees the full body. werk365 mutates the request method to work around this; verify we do not need to.

If the test fails, the fix is to hash `$response->getContent()` before Symfony empties it, which the middleware already does — investigate before changing anything, and do not copy werk365's mutate-and-restore approach without a failing test that demands it.
