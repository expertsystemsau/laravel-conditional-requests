...
This was expected to pass without production changes, on the assumption that `Router::runRoute()` calls `prepareResponse()` *after* the route middleware stack unwinds. That assumption was wrong: for route-scoped middleware, `runRouteWithinStack()`'s pipeline destination *is* `prepareResponse()` — it runs as the innermost call, before control returns to any route middleware's post-`$next()` code. Symfony's `Response::prepare()` nulls the body for `HEAD` there, so by the time `Conditional::handle()` inspects the response, there is nothing left to hash.
