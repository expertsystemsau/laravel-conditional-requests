...
- **Distinguishing "the strategy could not answer" from "the resource is absent"** through a new contract method. Both collapse to a `null` validator, and Task 4 shows the collapse is harmless for `If-None-Match: *` and fail-closed for `If-Match`. A tri-state existence contract would be a public API addition serving one documented caveat, and `lock` mode in v0.5 has to revisit resource resolution anyway — better to design it once, with that phase's requirements in hand.
