...
Hence `lockTarget()` and `lockAndRefresh()` on the contract, and `lockingQuery()` public on `ModelStrategy` — a genuine extension point (override it to add a shared lock, a scope, or a `->with()`) that happens also to be the seam the grammar test needs.
...
- **`sharedLock()` as an option.** `If-Match` guards a write, and a shared lock does not prevent another writer from taking an exclusive one after we release. It would be a weaker guarantee wearing the same flag. `ModelStrategy::lockingQuery()` is public so an application that genuinely wants one can override it.
