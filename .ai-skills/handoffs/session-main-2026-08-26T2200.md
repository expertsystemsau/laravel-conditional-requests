...
source: xsys-handoff
mode: full
generated_at: "2026-08-26T22:00+10:00"
title: "v1.0.1 released to Packagist"
status: ready-for-review
branch: "main"
head_sha: "cf3b2e1e0e03a064227047171061d3692ab991b7"
dirty_files: 2
diff_digest: "5cd8bd441b1d"
ticket_key: "none"
repo: "laravel-conditional-requests"
submodules: []
next_step: "Nothing is blocking. Shipped and verified. See Warnings before running composer analyse locally."
---

# Handoff: v1.0.1 released to Packagist
...
- [x] **v1.0.0 and v1.0.1 tagged and released**, both named exactly, both full
      releases, bodies pasted from the changelog rather than generated.
- [x] **v1.0.1 is a documentation-only patch** fixing hazard H6, which told the
      reader to set `Cache-Control` from middleware declared *inside*
      `conditional` — the one position that never executes on the short-circuit
      H6 is about. `docs/placement.md` carried the same inversion twice and
      `docs/api.md` undercounted the API-freeze examples. Found by re-running
      pre-flight step 5. Verified present in the shipped v1.0.1 dist and absent
      from v1.0.0's.
- [x] `1.x` moved forward to `v1.0.1`; both workflows green on that branch, so
      the `'*.x'` trigger is exercised rather than merely configured.
...
- [ ] Nothing is blocking, and nothing is known broken. The package is shipped.
...
- [ ] `~/projects/lcr-testbed` predates this session and was left alone. The
      scratch apps this session created, and their databases, are gone.
...
- **Expecting `changelog-updater-action` to move the `[Unreleased]` body.** It
  ...
  existing `[Unreleased]` content in place. This duplicated the content on both
  releases — expect the tidy every time, and see `.github/RELEASING.md` §2, which
  now describes the real behaviour instead of blaming a pre-written heading.
  The links are the part that improved: the action appends to whatever URL the
  `[Unreleased]` heading carried, so with `commits/main` in place v1.0.0 got
  `…/commits/main/compare/v1.0.0...HEAD` and
  `…/commits/main/compare/main...v1.0.0`, both 404 — but once `[Unreleased]`
  carried a real `…/compare/vX.Y.Z...main` base, v1.0.1's links came out correct
  on their own and only needed `HEAD` → `main`.

- **Trusting a delegated audit's verdict without checking it.** The first
  documentation-audit subagent died on a 529 and a second one went idle without
  reporting; the release went out on an audit done inline instead. The report
  then arrived late and was right on both counts — the H6 inversion is real and
  was verified against `Conditional.php` before anything was changed. Two
  lessons, opposite directions: a subagent going quiet is not a clean bill of
  health, and a late report is still worth reading.
...
**Working** — `main` at `cf3b2e1`, pushed. `tests` and `locking` green on `main`
and on `1.x`. **v1.0.1 is live on Packagist and installable**, and is what
`composer require expertsystemsau/laravel-conditional-requests` resolves to.
...
| Command | Result |
|---|---|
| `tests` + `locking` on `main` and on `1.x` | green |
| `tests` workflow | 24/24 legs, including 12 `windows-latest` |
| `locking` workflow | mysql 5 passed, pgsql 5 passed; the workflow's own guard fails on a skip, so this is proof it executed |
...
| Post-release install from Packagist, v1.0.1 | resolves; ETag then `304`, 0 body bytes; corrected H6 present in the dist, old wording absent |
